  ...
  return (
    <div className="space-y-12 pb-16">
      ...
      <section className="space-y-6">
        <h2 className="font-display text-4xl font-black text-foreground">The Vocabulary</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[
            { term: 'Neat', def: 'Room temp. No ice. Nothing else.', color: 'bg-card text-foreground' },
            { term: 'Up', def: 'Chilled & strained. No ice in glass.', color: 'bg-card text-foreground' },
            { term: 'Rocks', def: 'Over ice.', color: 'bg-card text-foreground' },
            { term: 'Dry', def: 'Little to no Vermouth.', color: 'bg-card text-foreground' },
            { term: 'Dirty', def: 'Add olive brine.', color: 'bg-card text-foreground' },
            { term: 'Double', def: '2x Spirit. Usually 3oz+.', color: 'bg-card text-foreground' },
            { term: 'Back', def: 'Side of water or soda.', color: 'bg-card text-foreground' },
            { term: '86\'d', def: 'Out of stock. Gone.', color: 'bg-[#ff5400] text-white' },
          ].map((item, idx) => (
            <div key={idx} className={`neo-card p-6 border-2 border-border shadow-neo-sm hover:shadow-neo-md transition-all ${item.color}`}>
